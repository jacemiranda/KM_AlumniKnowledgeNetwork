        ...
        <article key={`${item.kind}-${item.id}`} className="rounded-2xl border border-white/10 bg-[#131b2e]/60 p-4 shadow-liquid backdrop-blur-2xl">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 line-clamp-3 text-xs text-slate-300">{item.content}</p>
              <p className="mt-2 text-[10px] text-slate-500">Reason: {item.reason}</p>
              <p className="mt-1 text-[10px] text-slate-500">{item.byline}</p>
            </div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <button
              ...
              disabled={isMutating}
              className="rounded-full border border-emerald-300/40 bg-emerald-300/15 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.1em] text-emerald-100 transition hover:bg-emerald-300/20 disabled:opacity-40"
            >
              Approve / Keep
            </button>
            <button
              ...
              disabled={isMutating}
              className="rounded-full border border-[#ffb4ab]/40 bg-[#ffb4ab]/20 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.1em] text-[#ffb4ab] transition hover:bg-[#ffb4ab]/25 disabled:opacity-40"
            >
              Remove / Delete
            </button>
          </div>
